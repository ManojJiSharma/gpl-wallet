...
export default function SendDialog({ open, onClose, publicKey, balanceInfo }) {
  ...
  const getTabs = (mint) => {
    if (mint?.equals(WUSDC_MINT)) {
      return [
        <Tab label="GPL WUSDC" key="gpl" value="gpl" />,
      ...
      return [
        <Tab label="GPL WUSDT" key="gpl" value="gpl" />,
      ...
      return [
        <Tab label="GPL USDC" key="gpl" value="gpl" />,
      ...
      const tabs = [
        <Tab label={`GPL ${swapCoinInfo.ticker}`} key="gpl" value="gpl" />,
      ];
      if (
...
function SendSplDialog({ onClose, publicKey, balanceInfo, onSubmitRef }) {
  const defaultAddressHelperText =
    !balanceInfo.mint || balanceInfo.mint.equals(WRAPPED_GEMA_MINT)
      ? 'Enter Gemachain Address'
      : 'Enter GPL token or Gemachain address';
      ...
      try {
        ...
        if (destinationAccountInfo.owner.equals(TOKEN_PROGRAM_ID)) {
          ...
          setAddressHelperText(
            `Destination is a Gemachain address: ${
