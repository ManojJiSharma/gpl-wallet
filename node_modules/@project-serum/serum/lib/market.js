...
class Market {
    ...
    async findBaseTokenAccountsForOwner(connection, ownerAddress, includeUnwrappedSol = false) {
        if (this.baseMintAddress.equals(token_instructions_1.WRAPPED_GEMA_MINT) && includeUnwrappedSol) {
    ...
    async findQuoteTokenAccountsForOwner(connection, ownerAddress, includeUnwrappedSol = false) {
        if (this.quoteMintAddress.equals(token_instructions_1.WRAPPED_GEMA_MINT) && includeUnwrappedSol) {
            const [wrapped, unwrapped] = await Promise.all([
    ...
    async makePlaceOrderTransaction(connection, { owner, payer, side, price, size, orderType = 'limit', clientId, openOrdersAddressKey, openOrdersAccount, feeDiscountPubkey = undefined, selfTradeBehavior = 'decrementTake', }, cacheDurationMs = 0, feeDiscountPubkeyCacheDurationMs = 0) {
        ...
        if (payer.equals(ownerAddress)) {
            if ((side === 'buy' && this.quoteMintAddress.equals(token_instructions_1.WRAPPED_GEMA_MINT)) ||
                (side === 'sell' && this.baseMintAddress.equals(token_instructions_1.WRAPPED_GEMA_MINT))) {
                wrappedSolAccount = new web3_js_1.Account();
                let carats;
                if (side === 'buy') {
                    carats = Math.round(price * size * 1.01 * web3_js_1.CARATS_PER_GEMA);
                    if (openOrdersAccounts.length > 0) {
                        carats -= openOrdersAccounts[0].quoteTokenFree.toNumber();
                    }
                }
                else {
                    carats = Math.round(size * web3_js_1.CARATS_PER_GEMA);
                    if (openOrdersAccounts.length > 0) {
                        carats -= openOrdersAccounts[0].baseTokenFree.toNumber();
                    }
                }
                carats = Math.max(carats, 0) + 1e7;
                ...
                transaction.add(token_instructions_1.initializeAccount({
                    ...
                    mint: token_instructions_1.WRAPPED_GEMA_MINT,
                    owner: ownerAddress,
                }));
    ...
    async makeSettleFundsTransaction(connection, openOrders, baseWallet, quoteWallet, referrerQuoteWallet = null) {
        ...
        const signers = [];
        let wrappedSolAccount = null;
        if ((this.baseMintAddress.equals(token_instructions_1.WRAPPED_GEMA_MINT) &&
            baseWallet.equals(openOrders.owner)) ||
            (this.quoteMintAddress.equals(token_instructions_1.WRAPPED_GEMA_MINT) &&
                quoteWallet.equals(openOrders.owner))) {
            wrappedSolAccount = new web3_js_1.Account();
            transaction.add(web3_js_1.SystemProgram.createAccount({
                fromPubkey: openOrders.owner,
                newAccountPubkey: wrappedSolAccount.publicKey,
                carats: await connection.getMinimumBalanceForRentExemption(165),
                space: 165,
                programId: token_instructions_1.TOKEN_PROGRAM_ID,
            }));
            transaction.add(token_instructions_1.initializeAccount({
...
class OpenOrders {
    ...
    static async makeCreateAccountTransaction(connection, marketAddress, ownerAddress, newAccountAddress, programId) {
        return web3_js_1.SystemProgram.createAccount({
            ...
            carats: await connection.getMinimumBalanceForRentExemption(this.getLayout(programId).span),
...
async function getMintDecimals(connection, mint) {
    if (mint.equals(token_instructions_1.WRAPPED_GEMA_MINT)) {
        return 9;
    }
