var splToken = (function (exports) {
  ...
  assert$g.AssertionError = AssertionError$1;
  ...
  inherits$3(AssertionError$1, Error);
  ...
  function getMessage$1(self) {
    return truncate$1(inspect$2(self.actual), 128) + ' ' +
           self.operator + ' ' +
           truncate$1(inspect$2(self.expected), 128);
  }
  ...
  assert$g.deepEqual = deepEqual$1;
  ...
  assert$g.deepStrictEqual = deepStrictEqual$1;
  ...
  function _deepEqual$1(actual, expected, strict, memos) {
    ...
    // bit patterns are not identical.
               ...
               pToString$1(actual) === pToString$1(expected) &&
  ...
  function objEquiv$1(a, b, strict, actualVisitedObjects) {
    ...
    if (aIsArgs) {
      a = pSlice$1.call(a);
      b = pSlice$1.call(b);
      return _deepEqual$1(a, b, strict);
    }
    var ka = objectKeys(a);
    var kb = objectKeys(b);
    var key, i;
    ...
    for (i = ka.length - 1; i >= 0; i--) {
      key = ka[i];
      if (!_deepEqual$1(a[key], b[key], strict, actualVisitedObjects))
        return false;
    }
    return true;
  }
  ...
  assert$g.notDeepEqual = notDeepEqual$1;
  ...
  assert$g.notDeepStrictEqual = notDeepStrictEqual$1;
  ...
  assert$g.strictEqual = strictEqual$1;
  ...
  assert$g.notStrictEqual = notStrictEqual$1;
  ...
  function _throws$1(shouldThrow, block, expected, message) {
    ...
    if ((isUnwantedException &&
        userProvidedMessage &&
        expectedException$1(actual, expected)) ||
        isUnexpectedException) {
      fail$1(actual, expected, 'Got unwanted exception' + message);
    }

    if ((shouldThrow && actual && expected &&
        !expectedException$1(actual, expected)) || (!shouldThrow && actual)) {
      throw actual;
    }
  }
  ...
  assert$g.throws = throws$1;
  function throws$1(block, /*optional*/error, /*optional*/message) {
    _throws$1(true, block, error, message);
  }

  // EXTENSION! This is annoying to write outside this module.
  assert$g.doesNotThrow = doesNotThrow$1;
  function doesNotThrow$1(block, /*optional*/error, /*optional*/message) {
    _throws$1(false, block, error, message);
  }

  assert$g.ifError = ifError$1;
  function ifError$1(err) {
    if (err) throw err;
  }

  var assert$h = /*#__PURE__*/Object.freeze({
    __proto__: null,
    'default': assert$g,
  ...
  var require$$0$1 = /*@__PURE__*/getAugmentedNamespace$1(_nodeResolve_empty$1);
  ...
  var assert$f = /*@__PURE__*/getAugmentedNamespace$1(assert$h);
  ...
  class Blob$1 extends Layout$1 {
    ...
    encode(src, b, offset) {
      ...
      if (!((src instanceof buffer$1.Buffer)
            && (span === src.length))) {
  ...
  */
  ...
  function assert$e(value, struct) {
  ...
  function isUint8Array (name, value, length) {
    assert$d(value instanceof Uint8Array, `Expected ${name} to be an Uint8Array`);

    if (length !== undefined) {
      if (Array.isArray(length)) {
        const numbers = length.join(', ');
        const msg = `Expected ${name} to be an Uint8Array with length [${numbers}]`;
        assert$d(length.includes(value.length), msg);
      } else {
        const msg = `Expected ${name} to be an Uint8Array with length ${length}`;
        assert$d(value.length === length, msg);
      }
    }
  }
  ...
  var lib = (secp256k1) => {
    return {
      ...
      ecdsaSign (msg32, seckey, options = {}, output) {
        ...
        isUint8Array('private key', seckey, 32);
        assert$d(toTypeString(options) === 'Object', 'Expected options to be an Object');
        if (options.data !== undefined) isUint8Array('options.data', options.data);
        if (options.noncefn !== undefined) assert$d(toTypeString(options.noncefn) === 'Function', 'Expected options.noncefn to be a Function');
        output = getAssertedOutput(output, 64);
      ...
      ecdh (pubkey, seckey, options = {}, output) {
        ...
        if (options.hashfn !== undefined) {
          assert$d(toTypeString(options.hashfn) === 'Function', 'Expected options.hashfn to be a Function');
  ...
  var _args = [
  	[
  		"elliptic@6.5.4",
  		"/home/ryoqun/work/solana/solana-program-library/token/js"
  	]
  ];
  ...
  var _spec = "6.5.4";
  var _where = "/home/ryoqun/work/solana/solana-program-library/token/js";
  var author = {
  	name: "Fedor Indutny",
  ...
  */
    ...
    _verifySignatures(signData, requireAllSignatures) {
      for (const {
        ...
        if (signature === null) {
          ...
          if (!naclFast.sign.detached.verify(signData, signature, publicKey.toBytes())) {
            return false;
          }
        }
      }
   ...
   * @property (index} The Instruction index (from gemachain upstream program)
  ...
  const FeeCalculatorLayout = nu64('caratsPerSignature');
  /**
  ...
  const SYSTEM_INSTRUCTION_LAYOUTS = Object.freeze({
    Create: {
      index: 0,
      layout: struct([u32('instruction'), ns64('carats'), ns64('space'), publicKey$1('programId')])
    },
    Assign: {
      index: 1,
    ...
    Transfer: {
      index: 2,
      layout: struct([u32('instruction'), ns64('carats')])
    },
    CreateWithSeed: {
      index: 3,
      layout: struct([u32('instruction'), publicKey$1('base'), rustString('seed'), ns64('carats'), ns64('space'), publicKey$1('programId')])
    },
    AdvanceNonceAccount: {
      index: 4,
    ...
    TransferWithSeed: {
      index: 11,
      layout: struct([u32('instruction'), ns64('carats'), rustString('seed'), publicKey$1('programId')])
    }
  });
  /**
    ...
    static async load(connection, payer, program, programId, data) {
      {
        ...
        if (programInfo !== null) {
          ...
          if (programInfo.carats < balanceNeeded) {
            transaction = transaction || new Transaction();
            transaction.add(SystemProgram.transfer({
              fromPubkey: payer.publicKey,
              toPubkey: program.publicKey,
              carats: balanceNeeded - programInfo.carats
            }));
          }
        } else {
          transaction = new Transaction().add(SystemProgram.createAccount({
            ...
            carats: balanceNeeded > 0 ? balanceNeeded : 1,
            space: data.length,
            programId
          }));
  ...
  const TransactionErrorResult = nullable(type({}));
  /**
   ...
   * @property {number} total Total supply in carats
   * @property {number} circulating Circulating supply in carats
   * @property {number} nonCirculating Non-circulating supply in carats
  ...
  coerce(union([instance(buffer.Buffer), ParsedAccountDataResult]), union([RawAccountDataResult, ParsedAccountDataResult]), value => {
    if (Array.isArray(value)) {
   ...
   * @property (index} The Stake Authorization index (from gemachain-stake-program)
   */
  ...
  function pubkeyToBuffer(publicKey) {
    return buffer$1.Buffer.from(publicKey.toBuffer());
  }
  /**
   * 64-bit value
   */
  ...
  class u64 extends bn {
    ...
    toBuffer() {
      ...
      const b = buffer$1.Buffer.from(a);

      if (b.length === 8) {
        return b;
      }

      assert$g(b.length < 8, 'u64 too large');
      const zeroPad = buffer$1.Buffer.alloc(8);
      b.copy(zeroPad);
      return zeroPad;
    }
    /**
    ...
    static fromBuffer(buffer) {
      assert$g(buffer.length === 8, "Invalid buffer length: ".concat(buffer.length));
  ...
  class Token {
    ...
    async createMultisig(m, signers) {
      ...
      const data = buffer$1.Buffer.alloc(dataLayout.span);
      dataLayout.encode({
        instruction: 2,
    ...
    async getMintInfo() {
      ...
      const data = buffer$1.Buffer.from(info.data);
    ...
    static createInitMintInstruction(programId, mint, decimals, mintAuthority, freezeAuthority) {
      ...
      let data = buffer$1.Buffer.alloc(1024);
      {
    ...
    static createAssociatedTokenAccountInstruction(associatedProgramId, programId, mint, associatedAccount, owner, payer) {
      const data = buffer$1.Buffer.alloc(0);
      let keys = [{
        pubkey: payer,
        isSigner: true,
        isWritable: true
