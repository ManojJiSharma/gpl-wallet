...
const FeeCalculatorLayout = BufferLayout__namespace.nu64('caratsPerSignature');
/**
...
const SYSTEM_INSTRUCTION_LAYOUTS = Object.freeze({
  Create: {
    index: 0,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), BufferLayout__namespace.ns64('carats'), BufferLayout__namespace.ns64('space'), publicKey('programId')])
  },
  Assign: {
    index: 1,
  ...
  Transfer: {
    index: 2,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), BufferLayout__namespace.ns64('carats')])
  },
  CreateWithSeed: {
    index: 3,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), publicKey('base'), rustString('seed'), BufferLayout__namespace.ns64('carats'), BufferLayout__namespace.ns64('space'), publicKey('programId')])
  },
  AdvanceNonceAccount: {
    index: 4,
  ...
  TransferWithSeed: {
    index: 11,
    layout: BufferLayout__namespace.struct([BufferLayout__namespace.u32('instruction'), BufferLayout__namespace.ns64('carats'), rustString('seed'), publicKey('programId')])
  }
});
/**
...
function makeWebsocketUrl(endpoint) {
  ...
  // we're directly trying to connect to gemachain-validator's ws listening port.
...
const VersionResult = superstruct.type({
  'gemachain-core': superstruct.string(),
...
const GetRecentBlockhashAndContextRpcResult = jsonRpcResultAndContext(superstruct.type({
  ...
  feeCalculator: superstruct.type({
    caratsPerSignature: superstruct.number()
  })
}));
...
class Connection {
  ...
  async confirmTransaction(signature, commitment) {
    ...
    if (response === null) {
      ...
      throw new Error(`Transaction was not confirmed in ${duration.toFixed(2)} seconds. It is unknown if it succeeded or failed. Check signature ${signature} using the Gemachain Explorer or CLI tools.`);
    }
   ...
   *   const signature = await connection.requestAirdrop(myAddress, CARATS_PER_GEMA);
  ...
  async requestAirdrop(to, carats) {
    const unsafeRes = await this._rpcRequest('requestAirdrop', [to.toBase58(), carats]);
...
const CARATS_PER_GEMA = 1000000000;
...
exports.CARATS_PER_GEMA = CARATS_PER_GEMA;
